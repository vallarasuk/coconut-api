// Status
const { storeProduct, productIndex, Location } = require('../db').models;
...
const list = async (req, res, next) => {
  try {
    ...
    if (sort !== 'product_name' && sort !== 'location') {
    ...
    const storeProductData = await storeProductService.findAndCount(query);

    for (let index = 0; index < storeProductData.rows.length; index++) {
      const value = storeProductData.rows[index];
      ...
      const data = {
        ...
        mrp: productIndex?.mrp ? productIndex?.mrp : '',
        required_quantity: value?.min_quantity - value?.quantity,
      };
      storeProductList.push(data);
    }
    //return response
    return res.json(200, {
      totalCount: storeProductData.count,
      currentPage: page,
      pageSize,
